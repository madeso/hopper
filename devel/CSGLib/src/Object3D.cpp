...
Object3D::Object3D(Solid * solid)
{
	...
	delete &verticesPoints;		// Temp변수 삭제...Fix Memory Leak
	delete &indices;
	delete &colors;
	delete verticesTemp;
}
...
void Object3D::splitFaces(Object3D * object)
{
	...
	if(getBound().overlap(object->getBound()))
	{
		...
		for(int i=0;i<getNumFaces();i++)
		{
			...
			// In java the referenced object isn't switched to something
			// else when i is changed.

			// The referenced object has been destroyed
			// It is still referenced here though, so the garbage collector will leave it alone for the time being.
			// I need to rewrite this perhaps, or change some bits, so that
			// if the thing is deleted, make sure it isn't referenced anymore!
...
void Object3D::splitFace(int facePos, Segment & segment1, Segment & segment2)
{
	...
	else if (startType == Segment::FACE && endType == Segment::FACE)
	{
		...
		if (mlFabs(segmentVector.x)<TOL && mlFabs(segmentVector.y)<TOL && mlFabs(segmentVector.z)<TOL)
		{
		...
		//double dot1 = mlFabs(segmentVector.dot(vertexVector));
		double dot1 = mlFabs(segmentVector * vertexVector);
		vertexVector = mlVector3D(endPos.x-face.v2->x, endPos.y-face.v2->y, endPos.z-face.v2->z);
		vertexVector.Normalise();
		//double dot2 = mlFabs(segmentVector.dot(vertexVector));
		double dot2 = mlFabs(segmentVector * vertexVector);
		vertexVector = mlVector3D(endPos.x-face.v3->x, endPos.y-face.v3->y, endPos.z-face.v3->z);
		vertexVector.Normalise();
		//double dot3 = mlFabs(segmentVector.dot(vertexVector));
		double dot3 = mlFabs(segmentVector * vertexVector);
