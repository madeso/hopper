...
Solid* cvCSGConverter::convertMesh2Solid(Ogre::Entity *pEntity, int ObjNum)
{
	// Ogre Mesh -> Solid
...
bool cvCSGConverter::convertSolid2Mesh(const std::string& Name, Solid *Solid, Ogre::Entity *Ent1, Ogre::Entity *Ent2)
{
	// Get Data
	...
	for(int k=0; k<mSubMeshCount; k++)
	{
		...
		pSubMesh->indexData->indexCount  = Count;
		pSubMesh->indexData->indexBuffer = HardwareBufferManager::getSingleton().createIndexBuffer(HardwareIndexBuffer::IT_16BIT, pSubMesh->indexData->indexCount, HardwareBuffer::HBU_STATIC_WRITE_ONLY, false);
	...
	// Set Bound Box
	pMesh->_setBounds( AxisAlignedBox( mBBMin, mBBMax ), false );
...
std::string cvCSGConverter::getSubMeshMaterialName(Ogre::Entity *Ent1, int SubMeshNo)
{
	return Ent1->getSubEntity(SubMeshNo)->getMaterialName();
}
...
void cvCSGConverter::getMeshInformation(const Ogre::MeshPtr mesh,
										size_t &vertex_count,
										Ogre::Vector3* &vertices,
										Ogre::Vector3* &uvs,
										size_t &index_count,
