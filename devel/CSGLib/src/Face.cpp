...
float Face::getArea()
{
	...
	//float B = xy.angle(xz); // Handy function! Angle between two vectors...
	float B = 0.0f;
...
void Face::rayTraceClassify(Object3D & object)
{
	...
	do
	{
		...
		for(int i=0;i<object.getNumFaces();i++)
		{
			...
			if( face.getNormal() == mlVector3DZero )			// Face의 노말벡터가 0 인 경우, Invalid Face라고 보고 건너뛴다.
				continue;
			...
			if(bIntersectResult)
			{
				...
				if(mlFabs(distance)<TOL && mlFabs(dotProduct)<TOL)
				{
				...
				else if(mlFabs(dotProduct)>TOL && distance>TOL)
				{
...
{
	double a, b, z;
	if((mlFabs(pointLine1.y-pointLine2.y)>TOL)&&(((point.y>=pointLine1.y)&&(point.y<=pointLine2.y))||((point.y<=pointLine1.y)&&(point.y>=pointLine2.y))))
	{
...
{
	double a, b, z;
	if((mlFabs(pointLine1.x-pointLine2.x)>TOL)&&(((point.x>=pointLine1.x)&&(point.x<=pointLine2.x))||((point.x<=pointLine1.x)&&(point.x>=pointLine2.x))))
	{
