...
public:
	...
	// Ogre Mesh -> Solid
	Solid*	convertMesh2Solid(Ogre::Entity *pEntity, int ObjNum);
	// Solid -> Ogre Mesh
	bool	convertSolid2Mesh(const std::string& Name, Solid *Solid, Ogre::Entity *Ent1, Ogre::Entity *Ent2);

	void	setBBMin(Ogre::Vector3 Min) { mBBMin = Min; }
	void	setBBMax(Ogre::Vector3 Max) { mBBMax = Max; }

protected:
	std::string getSubMeshMaterialName(Ogre::Entity *Ent1, int SubMeshNo);
